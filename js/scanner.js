// js/scanner.js (DEFINITIVE FIX - Friday 2:45 PM)
...
/**
 * Processes a video frame using a robust pipeline and a reliable inversion method.
...
function _processFrame(canvas) {
    ...
    try {
        ...
        // Pipeline steps 1-3: Grayscale -> Blur -> Enhance Contrast
        cv.cvtColor(src, gray, cv.COLOR_RGBA2GRAY);
        cv.GaussianBlur(gray, blurred, new cv.Size(5, 5), 0, 0, cv.BORDER_DEFAULT);
        clahe = new cv.CLAHE(2.0, new cv.Size(8, 8));
        clahe.apply(blurred, enhanced);
        
        // Step 4: Create a clean white-on-black image using Otsu's method
        cv.threshold(enhanced, dst, 0, 255, cv.THRESH_BINARY_INV + cv.THRESH_OTSU);
        
        // Step 5: Dilate the white text to make it solid
        ...
        // --- THE DEFINITIVE FIX ---
        // Conditionally invert the image using the reliable cv.threshold method.
        if (config.opencv.invertFinal) {
            // This will produce a black-on-white image
            cv.threshold(dst, dst, 127, 255, cv.THRESH_BINARY_INV);
        }
        // If invertFinal is false, the image remains white-on-black as intended.
...
// The rest of the file (start, stop, etc.) is unchanged and correct.
